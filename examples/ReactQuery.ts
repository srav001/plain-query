...
function useQuery<T, Args extends any[] = []>(
  options: QueryOptions<T, Args>,
): QueryResult<T, Args> {
  const dataStore = useRef(createValueStore<T | undefined>(undefined));
  const loadingStore = useRef(
    createValueStore(options.initial?.manualFetch === true ? false : true),
  );
...
type StoreResult<T, Args extends any[]> = Omit<
  ReturnType<typeof useQuery<T, Args>>,
  "error" | "updateKeys"
> &
...
function useStore<T, Args extends any[] = []>(
  options: StoreOptions<T, Args>,
): StoreResult<T, Args> {
  const dataStore = useRef(createValueStore<T | undefined>(undefined));
  const loadingStore = useRef(
    createValueStore(options.initial?.manualFetch === true ? false : true),
