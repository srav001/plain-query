...
export class QueryClient<T, Args extends any[] = []> {
  ...
  private async fetchData(
    type: "initial" | "refresh" | "refetch" | "normal",
    ...args: Args
  ): Promise<T | undefined> {
    if (this.l) {
      return;
      ...
      try {
        ...
        if (this.fetchedOnce === true) {
          if (type === "refetch") {
            ...
            const cachedData = await this.setFromCache();
            if (cachedData) {
        ...
        this.lastArgs = args;
        const fetchedData = await this.options.fn(...args, controller.signal);

        this.onData(fetchedData);
        this.options.cacheAdapter.set(
          this.currentKey,
          add_ttl_for_cache(fetchedData, this.options.cacheTime),
        );
  ...
  public refresh(...args: Args): Promise<T | undefined> {
    const shouldReplaceActiveRequest = this.l;
    if (shouldReplaceActiveRequest) {
...
export class MutationClient<T, V> implements MutationResult<T, V> {
  ...
  constructor(initial: T | undefined, options: MutationOptions<T, V>) {
    ...
    if (initial) {
      this.options.cacheAdapter.set(
        this.cacheKey,
        add_ttl_for_cache(initial, this.options.cacheTime),
      );
    }
  }
  ...
  public async mutate(value: V | Mutator<V>): Promise<T | undefined> {
